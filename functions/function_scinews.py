...
def generate_answer_1shot(model, tokenizer, random_example1, sample, max_length, beams):
    inputs_dict = tokenizer(inference_prompt_1shot("<s>", random_example1, sample), truncation=True, max_length=max_length, return_tensors="pt")
    input_ids = inputs_dict.input_ids.cuda()
    predicted_ids = model.generate(input_ids, do_sample=False, repetition_penalty=1.2, no_repeat_ngram_size=3, max_new_tokens = 768, num_beams=beams)[0]
    prediction = tokenizer.decode(predicted_ids, skip_special_tokens=False)
    sample["Prediction"] = prediction
...
def generate_answer_2shot(model, tokenizer, random_example1, random_example2, sample, max_length, beams):
    inputs_dict = tokenizer(inference_prompt_2shot("<s>", random_example1, random_example2, sample), truncation=True, max_length=max_length, return_tensors="pt")
    input_ids = inputs_dict.input_ids.cuda()
    predicted_ids = model.generate(input_ids, do_sample=False, repetition_penalty=1.2, no_repeat_ngram_size=3, max_new_tokens = 768, num_beams=beams)[0]
    prediction = tokenizer.decode(predicted_ids, skip_special_tokens=False)
    sample["Prediction"] = prediction
...
def generate_answer_4shot(model, tokenizer, random_example1, random_example2, random_example3, random_example4, sample, max_length, beams):
    inputs_dict = tokenizer(inference_prompt_4shot("<s>", random_example1, random_example2, random_example3, random_example4, sample), truncation=True, max_length=max_length, return_tensors="pt")
    input_ids = inputs_dict.input_ids.cuda()
    predicted_ids = model.generate(input_ids, do_sample=False, repetition_penalty=1.2, no_repeat_ngram_size=3, max_new_tokens = 768, num_beams=beams)[0]
    prediction = tokenizer.decode(predicted_ids, skip_special_tokens=False)
    sample["Prediction"] = prediction
