...
def model_and_tokenizer_setup_quantization(base_model,quant_config,  peft_config):
    try:
...
def train_model_with_model_performance(dataset, model, tokenizer, training_args, max_length):
    training_args = TrainingArguments(**training_args)
    tokenized_datasets = dataset.map(
        tokenize_function(tokenizer, lambda x: training_prompt("<s>", x, "</s>"), max_length)
    )
